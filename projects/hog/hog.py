...
def always_roll(n):
    ...
    "*** YOUR CODE HERE ***"
    # END PROBLEM 6
    def current_status(score,opponent_score):
        return n
...
def make_averaged(original_function, total_samples=1000):
    ...
    3.0
    """
    def average_and_print(*args):
        sum_die = 0
        i = 0
        while i < total_samples :
            die = original_function(*args)
            sum_die = sum_die + die
            i += 1
        return sum_die / total_samples
    return average_and_print
    # BEGIN PROBLEM 8
    "*** YOUR CODE HERE ***"
    # END PROBLEM 8
...
def max_scoring_num_rolls(dice=six_sided, total_samples=1000):
    ...
    max_num = 1
    while t < 11 :
        this_turn = make_averaged(roll_dice, total_samples)(t, dice)
        if this_turn > max_sum:
            max_sum = this_turn
            max_num = t
            t += 1
        else:
...
def tail_strategy(score, opponent_score, threshold=12, num_rolls=6):
    ...
    # END PROBLEM 10
    current_gain = tail_points(opponent_score)
...
def square_strategy(score, opponent_score, threshold=12, num_rolls=6):
    ...
    if current_gain >= threshold:
        ...
        if perfect_square(score + current_gain) and next_perfect_square(score + current_gain)-score >= threshold:
            return 0
        return num_rolls
