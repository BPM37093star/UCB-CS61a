    ...
    def add_insect(self, insect):
        ...
        if insect.is_ant:
            if self.ant is None:
                ...
                # BEGIN Problem 9
                if self.ant.is_container is True and insect.is_container is False and self.ant.can_contain(insect):
                    self.ant.contain_ant(insect)
                elif self.ant.is_container is False and insect.is_container is True:
...
class Bee(Insect):
    ...
    def action(self, colony):
        ...
        # BEGIN EC
        if self.is_backwards and self.place.entrance is not Hive:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        distan = 0
        while distan < self.min_range:
            search_place = search_place.entrance
            distan += 1
        while distan < self.max_range:
            if len(search_place.bees) != 0:
                break
            elif search_place.entrance is hive:
                break
            else:
                search_place = search_place.entrance
                distan += 1
...
# END Problem 13
    ...
    def action(self, colony):
        ...
        if not self.is_true_queen:
            ...
            while double_place is not None:
                if double_place.ant:
                    if hasattr(double_place.ant, 'damage') and double_place.ant.doubled is False:
                        double_place.ant.damage *= 2
                        double_place.ant.doubled = True
                    if double_place.ant.is_container and double_place.ant.contained_ant and double_place.ant.contained_ant.doubled is False and hasattr(double_place.ant.contained_ant, 'damage'):
                        double_place.ant.contained_ant.damage *= 2
                        double_place.ant.contained_ant.doubled = True
...
def apply_effect(effect, bee, duration):
    ...
    if effect.__name__ == 'make_slow':
        ...
        bee.slow_action += duration
    if effect.__name__ == 'make_scare':
        bee.scare_action = duration
        bee.action = effect(bee.action, bee)
